...
begin
  if not exists (
    select 1 from pg_policies where tablename = 'drivers' and policyname = 'Allow all'
  ) then
    execute 'create policy "Allow all" on public.drivers for all using (true) with check (true)';
  end if;
end $$;

-- Wire the FK on vehicles → drivers
do $$
begin
  if not exists (
...
-- Seed drivers (skip if already present)
