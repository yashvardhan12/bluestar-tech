...
function Toast({ toast, onDismiss }: { toast: ToastItem; onDismiss: (id: string) => void }) {
  ...
  // Auto-dismiss — success only. An error that vanishes before it is read
  ...
  return (
    <div
      ...
      className={clsx(
        'w-[360px] bg-white border rounded-xl p-4 relative',
    ...
    >
      <div className="flex gap-4 items-start pr-8">
        {/* Status icon with halos — green check, or red alert on failure */}
        <div className="relative size-5 shrink-0 mt-0.5">
          ...
          {/* Icon */}
          {isError
            ? <AlertCircle className="absolute inset-0 size-5 text-red-600" strokeWidth={1.75} />
